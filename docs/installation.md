...
`<plugin-root>/<GOOS>/<GOARCH>`. v0.2.0 publishes native packages for
...
./scripts/checksum.sh
(cd dist && grep '  deepseek-vision_0.2.0_linux_amd64.zip$' checksums.txt | sha256sum -c -)
...
unzip -o dist/deepseek-vision_0.2.0_linux_amd64.zip -d "$plugin_dir"
...
`deepseek-vision-v0.2.0.so`, and pins the host selection with `store.version`.
...
tmp_dir=$(mktemp -d)
(cd dist && grep '  deepseek-vision_0.2.0_linux_amd64.zip$' checksums.txt | sha256sum -c -)
unzip -q dist/deepseek-vision_0.2.0_linux_amd64.zip -d "$tmp_dir"
rm -f "$plugin_dir/deepseek-vision.so" "$plugin_dir"/deepseek-vision-v*.so
install -m 0755 "$tmp_dir/deepseek-vision.so" "$plugin_dir/deepseek-vision-v0.2.0.so"
rm -rf "$tmp_dir"
```

Keep `store.version` equal to the filename version (the host accepts either
`0.2.0` or `v0.2.0` in configuration and normalizes it). Do not leave an
...
`/linux/amd64/deepseek-vision-v0.2.0.so`, the metadata version must be
`0.2.0`, and the config response must show `store.version: "0.2.0"`.
